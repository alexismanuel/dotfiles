...
```"""
    ...
    def get_commits_since_main(self) -> List[Dict]:
        ...
        try:
            # Find the merge base with main/master (try master first per repo convention)
            candidates = ['master', 'origin/master', 'main', 'origin/main']
            main_hash = None
            for branch in candidates:
                main_hash = self.run_git_command(["merge-base", "HEAD", branch])
                if main_hash:
                    break
    ...
    def generate_mr_title(self, jira_ticket: Optional[str] = None, non_interactive: bool = False) -> str:
        """Generate MR title in format RD-[ticket] [title]"""
        if not jira_ticket:
            if non_interactive:
                print("❌ Jira ticket number is required for MR title (use --jira)")
                sys.exit(1)
        ...
        if ticket_info and 'summary' in ticket_info:
            ...
            # In non-interactive mode, use branch name or generic title
            branch = self.get_current_branch()
            ticket_title = branch.replace('-', ' ').replace('_', ' ').title()
        else:
    ...
    def create_mr_with_glab(self, description: str, title: Optional[str] = None, jira_ticket: Optional[str] = None, non_interactive: bool = False) -> bool:
        """Create MR using glab CLI with confirmation"""
        try:
            # Generate title if not provided
            if not title:
                title = self.generate_mr_title(jira_ticket, non_interactive)

            # Show preview and ask for confirmation (skip in non-interactive mode)
            if not non_interactive and not self.confirm_mr_creation(title, description):
...
def main():
    ...
    if args.create and not jira_ticket:
        if args.non_interactive:
            print("❌ Jira ticket number is required for MR creation (use --jira)")
            sys.exit(1)
        ...
        if not response:
            ...
            sys.exit(1)
        jira_ticket = response
    elif not jira_ticket and not args.non_interactive:
    ...
    if args.create:
        ...
        success = generator.create_mr_with_glab(description, args.title, jira_ticket, args.non_interactive)
